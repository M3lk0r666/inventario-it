<x-mail.shell :companyName="$companyName" subtitle="Recepción de bienes registrada" :supportEmail="$supportEmail" :accent="$accent">
    @if (! empty($toManager))
        <p style="font-size:15px;margin:0 0 14px;">Hola <strong>{{ $managerName }}</strong>,</p>
        <p style="margin:0 0 22px;">
            Te informamos, como jefe inmediato, que tu colaborador <strong>{{ $employee->name }}</strong>
            realizó la <strong>devolución</strong> de los siguientes bienes con fecha
            <strong>{{ \Illuminate\Support\Carbon::parse($returnedAt)->format('d/m/Y') }}</strong>@if ($folio),
            amparada por la carta de recepción con folio <strong style="color:{{ $accent }};">{{ $folio }}</strong>@endif.
        </p>
    @else
    ...
    @if ($assignments->isNotEmpty())
        <div style="font-size:16px;font-weight:700;color:{{ $accent }};margin:0 0 12px;">Equipos recibidos</div>
    ...
    @if ($items->isNotEmpty())
        <div style="font-size:16px;font-weight:700;color:{{ $accent }};margin:0 0 10px;">Bienes adicionales recibidos</div>
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:6px;">
        <tr>
            <td style="background:#eef3fb;border-left:4px solid {{ $accent }};border-radius:8px;padding:14px 16px;font-size:13px;line-height:1.6;color:#33415c;">
                @if (! empty($toManager))
                    Con esto se libera la responsabilidad del colaborador sobre los bienes listados. Este aviso es únicamente informativo.
                @else
                    {!! nl2br(e($note)) !!}
                @endif
            </td>

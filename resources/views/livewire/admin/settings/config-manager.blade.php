<div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[220px,1fr] gap-gutter items-start">
        ...
        <div class="card p-6 max-w-2xl">
            ...
            {{-- CARTAS --}}
            @elseif ($tab === 'letters')
                <h3 class="text-title-md text-on-surface mb-4">Cartas responsivas</h3>
                <div class="space-y-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="form-label">Prefijo de folio</label>
                            <input type="text" wire:model="letter_folio_prefix" class="form-input" @disabled($readonly)>
                            @error('letter_folio_prefix') <p class="form-error">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="form-label">Siguiente consecutivo</label>
                            <input type="number" min="1" wire:model="letter_next_number" class="form-input" @disabled($readonly)>
                            @error('letter_next_number') <p class="form-error">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    <p class="form-help">Formato del folio: {{ $letter_folio_prefix }}-{{ now()->year }}-{{ str_pad($letter_next_number ?: '1', 4, '0', STR_PAD_LEFT) }}</p>
                    <div>
                        <label class="form-label">Texto introductorio de la carta</label>
                        <textarea wire:model="letter_intro_text" rows="4" class="form-input" @disabled($readonly)></textarea>
                        @error('letter_intro_text') <p class="form-error">{{ $message }}</p> @enderror
                    </div>
                    @unless ($readonly)
                        <div class="flex justify-end border-t border-border-soft pt-4">

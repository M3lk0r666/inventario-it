...
### Corregir / anular asignación (2026-08-02)
Desde **Cartas responsivas** (vista agrupada), en las cartas de **entrega generadas sin devoluciones**:
- **Corregir** (icono lápiz, permiso `assignments.edit`): reabre `AssignmentForm` en modo edición (`editingLetterId`, evento `edit-assignment`) con los datos de la carta; permite cambiar empleado, activos (agrega/quita liberando o asignando), estado físico, fecha, notas y bienes adicionales, y **regenera el PDF conservando el mismo folio** (`saveEdit`). No re-notifica por correo por defecto. El form se incluyó también en `admin/letters/index`.
- **Anular** (mejorado): además de marcar la carta como anulada (marca ANULADA, folio conservado), si es de **entrega** ahora **libera los activos** aún asignados (los pasa al primer estado asignable y borra —soft— sus asignaciones). Toast indica cuántos se liberaron. `LettersByEmployee` agrega `returned_count` para habilitar Corregir solo si no hay devoluciones. **No requiere migración**; `view:clear`.

### Ajustes plantillas + fix sidebar (2026-08-02)
- **Fix resaltado del sidebar**: rutas con prefijo común (`admin.settings.*`) marcaban varias opciones activas a la vez. Se agregó soporte de `'match'` explícito por item; "Configuración" usa `match => 'admin.settings.index'` para resaltarse solo a sí misma.
- **Plantillas de cartas** movidas fuera de Configuración a su propio menú **Administración → Plantillas de cartas** (`admin.settings.letter-templates`): nuevo componente `LetterTemplatesManager` (folio + texto CAB/CEB con la protección de consecutividad `clampStart`). Se quitó la pestaña "Cartas responsivas" de `ConfigManager` (quedan Empresa y Correo).

### Plantillas de correo configurables (2026-08-02)
Nueva página **Administración → Plantillas de correo** (`admin.settings.email-templates`, permiso settings.view/edit) para personalizar los correos de aviso sin tocar código. `App\Support\MailTemplates` centraliza: color de acento (`mail_accent_color`), texto del pie (`mail_footer_text`) y, por correo, **asunto/introducción/nota** con overrides en settings (`mail_tpl_{key}_{field}`) y textos por defecto; `render()` sustituye variables `{empresa}`,`{empleado}`,`{jefe}`,`{folio}`,`{fecha}`,`{correo}`,`{rol}`,`{portal}`. Correos editables: asignación, recepción, acceso al portal, revocación. El `<x-mail.shell>` toma el acento y el pie de estos settings; cada plantilla usa `{{ $accent }}` en títulos/notas y `{!! nl2br(e($intro/$note)) !!}`. Las tablas de equipos y el saludo "Hola {empleado}," siguen automáticos; la copia al jefe conserva su redacción fija. Componente `EmailTemplatesManager` con botón "Restaurar" por correo. **No requiere migración**; solo `view:clear`.

### Jefe inmediato del empleado (2026-08-01)
Nueva auto-referencia `employees.manager_id` (migración `..._000100_add_manager_to_employees`, nullOnDelete). Modelo: relaciones `manager()` / `subordinates()`. `EmployeeForm`: select "Jefe inmediato" (empleados activos, no uno mismo) + validación. Ficha de empleado (Datos): muestra "Jefe inmediato". **Cartas responsivas**: el PDF incluye "Jefe Inmediato" en Información del Colaborador; nuevo placeholder `{jefe_inmediato}` (eager-load `employee.manager`). **Correos**: al asignar y al recibir, además del empleado se envía **copia informativa al jefe inmediato** (`AssignmentNotificationMail`/`ReceptionNotificationMail` con flag `toManager` que cambia saludo/redacción). En ambos formularios: bloque "Jefe inmediato: <nombre>" + casilla "Notificar al jefe inmediato" (default sí; se desactiva si no hay jefe o sin correo). **Requiere `php artisan migrate` + `npm run build` + `view:clear`.**

### Fix: Cartas responsivas en blanco (2026-08-01)
Regresión introducida por el sidebar colapsable: se había puesto `x-data` (vacío) en el contenedor `.main-content` de `layouts/admin.blade.php` para el margen; eso metió el `{{ $slot }}` en un scope Alpine y rompió la página de Cartas (única que usa `x-data="{ view: 'grouped' }"` a nivel `<x-admin-layout>`): al no estar `view` en ese scope, ambas vistas (agrupada/listado) quedaban ocultas → página vacía (las cartas SÍ se generaban). Fix: el contenedor ahora hace `{{ $attributes->merge(...) }}` (así el `x-data` de la página aterriza ahí y el toggle funciona) y el **margen del sidebar se movió a CSS** con `body:has(aside.sidebar-collapsed) .main-content { margin-left:4rem }` (ya no depende de Alpine). **Requiere `npm run build` + `view:clear`.**

### Más notificaciones por correo (2026-08-01)
- **Recepción de bienes**: `ReceptionForm` ahora envía aviso al empleado (`App\Mail\ReceptionNotificationMail` + `emails/reception.blade.php`, mismo shell), con casilla `notifyEmployee` (default sí) y el mismo manejo de correo no configurado / sin correo. Lista equipos recibidos + bienes adicionales; referencia el folio si hay carta.
- **Revocación de acceso al portal**: al revocar desde la ficha del empleado se notifica al usuario (`App\Mail\PortalAccessRevokedMail` + `emails/portal-revoked.blade.php`). `PortalAccessService::sendRevoked($name,$email)` (se capturan nombre/correo antes de borrar la cuenta); `EmployeeDetail::revokeAccess()` ahora recibe el servicio por DI y lanza toast del resultado. Solo `view:clear` al desplegar.

### Diseño unificado de correos (2026-08-01)
Nuevo componente `resources/views/components/mail/shell.blade.php` (`<x-mail.shell :companyName :subtitle :supportEmail :width :footnote>`): tarjeta redondeada con sombra, encabezado azul con distintivo (solo bordes, sin fuente de iconos), cuerpo (slot) y pie con "Contacto TI" (`mail_from_address`). Maquetado con tablas. Adoptado por: **asignación** (`emails/assignment`), **acceso al portal** (`emails/welcome-user`, con footnote propio y aviso ámbar de vigencia 24h) y **artículo de KB** (`emails/kb-article`, `width=820` por ser texto largo). Los Mailables respectivos pasan ahora `supportEmail`. Solo `view:clear` al desplegar.

### Aviso por correo al asignar bienes (2026-08-01)
Al registrar una asignación que genera carta, se puede **notificar por correo al empleado** (versión digerible de la carta): `App\Mail\AssignmentNotificationMail` + plantilla `emails/assignment.blade.php` (diseño tipo tarjeta redondeada con sombra, encabezado azul con distintivo, tabla de equipos con franjas, lista de bienes adicionales, recuadro de aviso de firma, y pie con contacto TI = `mail_from_address`). Maquetada con tablas para compatibilidad de correo. En `AssignmentForm`: casilla `notifyEmployee` (default sí, solo si el empleado tiene correo y el correo está configurado — se avisa en el form) y método `sendAssignmentEmail()` que usa `MailConfigurator::isReady()/apply()` + `Mail::to(...)->send(...)`, con toast de éxito/error. Solo se envía cuando se genera carta. **No requiere migración**; sí `npm run build` + `view:clear`. El correo se configura en Configuración → Correo (O365).
